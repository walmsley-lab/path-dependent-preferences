...
**Path-dependent preferences (the term).** The phrase has an established meaning in behavioral economics, where early experiences and transaction histories alter subsequently expressed preferences or fairness judgments (e.g., work on early experience and biased search in preference development). Our use is related but distinct: we hold the training examples and their frequencies fixed and intervene only on their *ordering*, asking whether different permutations of identical experience select different computational mechanisms underlying otherwise matched behavior. Humans can exhibit path-dependent preferences partly because early experience changes *what they subsequently encounter*; our design removes that channel entirely — same learner, same evidence, same amount of evidence, different order. The intervention concerns developmental history rather than differences in observed evidence. (Submission title should disambiguate from the economics literature: leading candidate *"Developmental Path Dependence in Language Models: Can Training Order Select the Mechanism Behind a Model's Preferences?"* — see §9b.)
