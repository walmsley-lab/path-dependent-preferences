...
**PROPOSED amendment (recorded before any batch data exists; requires Patrick's explicit approval before launch):** the Override-A directional gate was designed to prevent launching into a *null*; the observed outcome is instead a large sign-consistent effect whose direction falsifies the auxiliary theory, not the primary hypothesis. Proposal: authorize the batch on the basis that the mini satisfies the *form* of the primary criterion (same-sign, |mean Δ| ≥ 10 pp) at n=2; the primary endpoint, contrast, and direction-agnostic support rule remain exactly as originally frozen; the directional conjecture is recorded as refuted; the batch's exploratory expectations are updated to the interference/recency hypothesis (C2 > C1 in utility-weight; C3 intermediate-to-high). If Patrick declines, the sprint result is the calibration story including this inverted-direction mini finding.
