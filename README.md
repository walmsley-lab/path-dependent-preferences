...
| 2½ | `python interact.py --run <run> --data <data> [--compare <run2>] [--probes]` | any, instant | **Meet the model organism:** interrogate a trained checkpoint — pose scenarios, render the *same* scenario aligned/conflict/no-cue, compare C1 vs C2 side by side, read the λ-probe beside the behavior. `export` feeds the (planned) browser explorer |
